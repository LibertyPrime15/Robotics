package org.firstinspires.ftc.Colee.leagueCode.blue;
...
@Autonomous(name = "blue Master", group = "blueLeague")
//@Disabled
...
//Power is the power that we want to move at; it should be between 0 and 1. Making this value negative will put the bot into a loop that goes on forever
	...
	private void blockPositionOne()
	{
		...
		robot.stopIntake();
		moveDistanceAtAngle(9, 20, 0.4);
		turnAngle(90, 2000);
		moveDistanceAtAngle(-75, 90, 0.6);
		turnAngle(0,2000);
		moveDistanceAtAngle(-10,0,.2);
		robot.grabPlate();
		sleep(1000);
		moonDrive(90,.6,.7,20000);
		sleep(500);
		moveDistanceAtAngle(-17,90,.5);
		sleep(500);
		robot.disengageIntake();
		moveDistanceAtAngle(10,90,.5);
		robot.ungrabPlate();
		turnAngle(135,1500);
		moveDistanceAtAngle(17,135,.5);
		turnAngle(90,1500);
		moveDistanceAtAngle(5,90,.6);
		stop();
	}
	//--------------------------------------------------------------------------------------------------
