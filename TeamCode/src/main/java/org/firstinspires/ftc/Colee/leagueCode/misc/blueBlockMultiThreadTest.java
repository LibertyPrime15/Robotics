...
@Autonomous(name = "BlueBlockMultiThreadTest", group = "Concept")
//@Disabled
public class blueBlockMultiThreadTest extends LinearOpMode
{
...
//--------------------------------------------------------------------------------------------------
public void passPosition(double xPos, double yPos)
{
	this.yPos = yPos;
	this.xPos = xPos;
}
//--------------------------------------------------------------------------------------------------
public double returnEncoderValues()
{
	int sum = 0;
	sum += robot.front_left.getCurrentPosition();
	sum += robot.back_right.getCurrentPosition();
	sum += robot.back_left.getCurrentPosition();
	sum += robot.front_right.getCurrentPosition();
...
public void moveDistanceAtAngle(double distance, double angle, double power)
{
	...
	double totalDistInSteps = (35 * distance) + (robot.front_right.getCurrentPosition() + robot.front_left.getCurrentPosition() + robot.back_left.getCurrentPosition() + robot.back_right.getCurrentPosition());
